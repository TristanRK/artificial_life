...
def sidebar_controls():
    ...
    dt = st.sidebar.slider("Time Step (dt)", min_value=0.01, max_value=0.1, value=0.02, step=0.005)
    r = st.sidebar.slider("Interaction Radius (r)", min_value=10, max_value=200, value=80, step=20)
    friction_factor = st.sidebar.slider("Friction Factor", min_value=0.1, max_value=1.0, value=0.3, step=0.1)
    beta = st.sidebar.slider("Beta (Force Calculation)", min_value=0.1, max_value=1.0, value=0.3, step=0.05)
